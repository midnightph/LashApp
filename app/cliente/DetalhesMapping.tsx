...
export default function DetalhesMapping({ navigation, route }: any) {
  ...
  const fetchTelefone = async () => {
    ...
    const refDoc = doc(database, "user", user.uid, "Clientes", clienteId);
  ...
  const excluir = async () => {
    ...
    try {
      ...
      const storagePath = item.fotoPath;
      if (
        storagePath !==
  ...
  const encurtarUrl = async (urlOriginal: string): Promise<string | null> => {
    try {
      const response = await fetch(
        `https://tinyurl.com/api-create.php?url=${encodeURIComponent(urlOriginal)}`
      );
  ...
  const tirarFoto = async () => {
    ...
    if (!resultado.canceled) {
      ...
      try {
        const { url, fullPath } = await uploadImagem(uri, clienteId, user.uid, id);
        await updateDoc(
          doc(database, "user", user.uid, "Clientes", clienteId, "Historico", id),
          {
            foto: url,
            fotoPath: fullPath,
          }
        );
        await updateDoc(doc(database, "user", user.uid, "Clientes", clienteId), {
          foto: url,
          fotoPath: fullPath,
        });
        setImagem(url);
    ...
    <>
      ...
      >
        ...
        <SafeAreaView style={styles.container}>
          ...
          >
            <Image source={{ uri: imagem }} style={styles.clientImage} />
